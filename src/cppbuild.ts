...
import {FileSystemWatcher, Memento, OutputChannel, Uri, WorkspaceConfiguration} from 'vscode';
...
var suf_map:
    Record<string, string[]> = {msbuild: [], make: [], xcodebuild: ['build']};
...
function build_project(proj: Project, params: string[]) {
  ...
  params = opt_map[build_tool]
               .concat([proj.proj])
               .concat(suf_map[build_tool])
               .concat(params);

  let options: Object = {};
  if (proj.path.length !== 0) {
...
function get_proj(path: string) {
  ...
  if (darwin() && build_tool === xcode) {
    let result: string[] =
        (<Buffer><any>spawn.sync(build_tool, ['-list', '-project', path])
             .stdout)
            .toString()
            .split('\n');
...
function itemize_proj(uri: string, proj: string) {
  if (darwin() && build_tool === xcode) {
    return Project(
        path.dirname(uri), proj,
        path.join(
            path.dirname(path.relative(vscode.workspace.rootPath, uri)), proj));
  }

  return Project('', uri, path.relative(vscode.workspace.rootPath, uri));
}
  ...
  return new Promise((fulfill, reject) => {
    ...
    if (vscode.workspace.rootPath !== undefined) {
      ...
      try {
        ...
        return load_proj_map().then((value) => {
          if (value !== undefined) {
            let projList: string[] = <string[]><any>value;

            let glob: string =
                '**/*.{' + build_ext.join(',').replace(/\./g, '') + '}';
...
function try_cfg(valid_tools: string[]) {
  let config: WorkspaceConfiguration = vscode.workspace.getConfiguration(
      global.TAG_CODECMDER + '.' + global.TAG_BUILDTOOL);
...
function load_ignore_cfg() {
  let config: WorkspaceConfiguration =
      vscode.workspace.getConfiguration(global.TAG_CODECMDER);
...
var trigger_build = function(state: Memento) {
  ...
  if (load_env()) {
    load(state).then((value) => {
      let projList: string[] = <string[]><any>value;

      if (projList !== undefined) {
        if (projList.length === 0) {
          vscode.window.showErrorMessage(
              'Could not find any C++ projects to build');
          return;
        }

        vscode.window.showQuickPick(projList).then((val: string) => {
          if (val === undefined) {
            return;
          }
