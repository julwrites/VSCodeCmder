...
function activate(context: ExtensionContext) {
  ...
  var jmpCommand =
      vscode.commands.registerCommand('codecmder.jumpToPath', () => {
        directory.chdir(state);
      });
  var addCommand =
      vscode.commands.registerCommand('codecmder.addBookmark', () => {
        bookmarks.add_bookmark(state);
      });
  var delCommand =
      vscode.commands.registerCommand('codecmder.removeBookmark', () => {
        bookmarks.del_bookmark(state);
      });
  var clrCommand =
      vscode.commands.registerCommand('codecmder.clearBookmarks', () => {
        bookmarks.clr_bookmarks(state);
      });
  var bldCommand =
      vscode.commands.registerCommand('codecmder.buildproj', () => {
